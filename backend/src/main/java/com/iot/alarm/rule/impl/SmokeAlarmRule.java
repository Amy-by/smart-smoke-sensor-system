 ...
 * @date 2025-12-10
 */
public class SmokeAlarmRule implements Rule {
    
    private Integer threshold;
    private String ruleName;
    private Integer priority;
    
    public SmokeAlarmRule(AlarmRule rule) {
    ...
    public Alarm execute(SensorData sensorData) {
        Integer smokeConcentration = sensorData.getSmokeConcentration();
        if (smokeConcentration != null && smokeConcentration >= threshold) {
            Alarm alarm = new Alarm();
            alarm.setDeviceId(sensorData.getDeviceId());
            alarm.setAlarmType("火灾");
            alarm.setAlarmLevel(getAlarmLevel(priority));
            alarm.setAlarmTime(LocalDateTime.now());
            alarm.setDescription(String.format("烟雾浓度超过阈值：%d%%（阈值：%d%%）", 
                smokeConcentration, threshold));
            return alarm;
        }
        return null;
    }
